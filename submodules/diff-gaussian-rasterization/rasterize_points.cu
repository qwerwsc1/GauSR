	...
	const bool debug)
{
  ...
  if (means3D.ndimension() != 2 || means3D.size(1) != 3) {
	...
	if (render_geo && (all_map.ndimension() != 2 || all_map.size(0) != means3D.size(0) || all_map.size(1) != NUM_ALL_MAP)) {
		AT_ERROR("all_map must have dimensions (num_points, 5) when render_geo is enabled");
	}
  ...
  return std::make_tuple(rendered, out_color, radii, out_all_map, out_plane_depth, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dall_map = torch::zeros_like(all_map);
