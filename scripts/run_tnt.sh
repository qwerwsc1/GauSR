...
for idx in "${!scenes[@]}"; do
    ...
    # python mesh_extract_tetrahedra.py -m ${output_folder}/${scene}
    python scripts/eval_tnt/run.py --dataset-dir ${dataset_folder}/${scene} --traj-path ${dataset_folder}/${scene}/${scene}_COLMAP_SfM.log --ply-path ${output_folder}/${scene}/recon.ply --out-dir ${output_folder}/${scene}/mesh
done